...
import '/perfil/deletar_visto_recente/deletar_visto_recente_widget.dart';
...
import 'visto_recente_model.dart';
export 'visto_recente_model.dart';

class VistoRecenteWidget extends StatefulWidget {
  const VistoRecenteWidget({super.key});

  static String routeName = 'VistoRecente';
  static String routePath = '/vistoRecente';

  @override
  State<VistoRecenteWidget> createState() => _VistoRecenteWidgetState();
}

class _VistoRecenteWidgetState extends State<VistoRecenteWidget> {
  late VistoRecenteModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => VistoRecenteModel());
                            ...
                            Padding(
                              ...
                              child: AuthUserStreamWidget(
                                builder: (context) => Builder(
                                  builder: (context) {
                                    final produtoRef = (currentUserDocument
                                                ?.vistoRecente
                                                .toList() ??
                                            [])
                                        .toList();
                                          ...
                                          children: [
                                            StreamBuilder<ProdutoRecord>(
                                              stream: ProdutoRecord.getDocument(
                                                  functions
                                                      .recomendarProdutosComprouAntes3(
                                                          produtoRefItem)),
                                              builder: (context, snapshot) {
                                                // Customize what your widget looks like when it's loading.
                                                if (!snapshot.hasData) {
                                            ...
                                            DeletarVistoRecenteWidget(
                                              key: Key(
                                                  'Keygl8_${produtoRefIndex}_of_${produtoRef.length}'),
                                              idFavorito: produtoRefItem,
                                            ),
                                          ],
                                        );
                                      },
